...
if ($_POST["submit"] === "true") {
    if (preg_match("/^\w{1,24}$/u", $_POST["login"]) and $_POST["password"] !== '') {
        $loginLog = $_POST["login"];
        $password = $_POST["password"];
        if ($connect->checkUser($loginLog, $password, "users")) {
            setcookie("login", $_POST["login"]);
            setcookie("password", sha1($_POST["password"]));
            header("Location:{$_SERVER['HTTP_REFERER']}");
        } else {
            $echo = 'Логін і пароль не збігаються';
        }
    } else {
        $echo = "У полях не коректні дані";
    }
}
$login = $_COOKIE["login"];
?>
        ...
        <div id="mid">
            ...
            <div id="posts">
                <div class="post">
                    ...
                    <div class="story">
                        <form action="Login.php" method="POST" enctype='multipart/form-data'>
                            <div><label class="task" style="width: 70px" for="login">Логін</label><input type="text" id="login" class="pole" name="login" required></div>
                            <div><label class="task" style="width: 70px" for="pass">Пароль</label><input type="password" id="pass" class="pole" name="password" required></div>
                            <input type="hidden" name="submit" value="true">
                            <input type="submit" value="Ввійти" style="margin-left: 3%;">
                            <input value="Зареєструватися" onclick="location.href = 'Registration.php'" type="button" style="margin-left: 1%;">
                        </form>
                    </div>
                </div>
            </div>
