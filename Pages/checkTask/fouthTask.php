        ...
        <div id="mid">
            ...
            <div id="posts">
                <div class="post">
                    <div class="story">
                        <?php
                        if (!$connect->isDone("4", $connect->getUserId($login, "users"), "taskdone")) {
                            $done = true;
                            $rezult = commaToPoint($_POST);
                            foreach ($rezult as $value) {
                                foreach ($value as $valuelow) {
                                    if ($valuelow[0] == "" or ! is_numeric($valuelow[0])) {
                                        $done = false;
                                    } if ($valuelow[1] == "" or ! is_numeric($valuelow[1])) {
                                        $done = false;
                                    }
                                }
                            }
                            if ($done) {
                                $data = $connect->getTaskRezult("4", $connect->getUserId($login, "users"), "tasktable");
                                $data = unserialize($data);
                                $arrayObject1 = new ArrayObject($data);
                                $arrayObject2 = new ArrayObject($rezult);
                                ...
                                for ($iterator1->rewind(), $iterator2->rewind(); $floatEt = $iterator1->current(), $floatCheck = $iterator2->current(); $iterator1->next(), $iterator2->next()) {
                                    $check[] = TaskCheck::fouthTaskCheck($floatEt, $floatCheck);
                                }
                                ...
                                if ($connect->taskCheckExist("4", $connect->getUserId($login, "users"), "taskdone")) {
                                    $connect->taskCheckUpdate("4", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                } else {
                                    $connect->taskCheck("4", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                }
                            } else {
                            ...
                            $done = $connect->isDone("4", $connect->getUserId($login, "users"), "taskdone");
                            echo 'Завдання виконано<br>';
                        }
                        if ($done) {
                            $dataStudent = $connect->getTaskRezult(4, $connect->getUserId($login, "users"), "taskdone");
                            $dataStudent = unserialize($dataStudent);
                            $dataSolver = $connect->getTaskRezult(4, $connect->getUserId($login, "users"), "tasktable");
                            ...
                            foreach ($dataSolver as $key => $value) {
                                ...
                                echo "<br><div>Дані для n = $key<div><br><table class='task'><tr class='task'><td class='task'>Метод</td>";
                                for ($index = 0; $index < count($xData); $index++) {
                                    echo '<td class="task" colspan="2">' . rule($rule, $xData[$index]) . "</td>";
                                }
                                echo "</tr><tr class='task'><td class='task'>Значення</td>";
                                for ($index = 0; $index < count($xData); $index++) {
                                    echo '<td class="task">x</td><td class="task">d</td>';
                                }
                                echo '</tr><tr class="task"><td class="task">Дані студента</td>';
                                for ($index = 0; $index < count($yDataStudent); $index++) {
                                    echo '<td class="task">' . round($yDataStudent[$index][0], 6) . "</td>";
                                    echo '<td class="task">' . round($yDataStudent[$index][1], 9) . "</td>";
                                }
                                echo '</tr><tr class="task"><td class="task">Відхилення від рішення</td>';
                                for ($index = 0; $index < count($yDataStudent); $index++) {
                                    echo '<td class="task">' . round(abs(abs($yDataStudent[$index][0]) - abs($yDataSolver[$index][0])), 8) . "</td>";
                                    echo '<td class="task">' . round(abs(abs($yDataStudent[$index][1]) - abs($yDataSolver[$index][1])), 12) . "</td>";
                                }
                                echo '</tr></table>';
                            }
                        }
