                    ...
                    <div class="story">
                        ...
                        if (!$connect->isDone("1", $connect->getUserId($login, "users"), "taskdone")) {
                            ...
                            if ($done) {
                                ...
                                for ($iterator1->rewind(), $iterator2->rewind(); $floatEt = $iterator1->current(), $floatCheck = $iterator2->current(); $iterator1->next(), $iterator2->next()) {
                                    $check[] = TaskCheck::firstTaskCheck($floatEt, $floatCheck);
                                }
                                ...
                                if ($connect->taskCheckExist("1", $connect->getUserId($login, "users"), "taskdone")) {
                                    $connect->taskCheckUpdate("1", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                } else {
                                    $connect->taskCheck("1", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                }
                            } else {
                        ...
                        if ($done) {
                            ...
                            foreach ($dataSolver as $key => $value) {
                                ...
                                echo '</tr><tr><td class="task">Відхилення від рішення</td>';
                                for ($index = 0; $index < count($yDataStudent); $index++) {
                                    echo '<td class="task">' . round(abs(abs($yDataStudent[$index]) - abs($yDataSolver[$index])), 7) . "</td>";
                                }
                                echo '</tr></table>';
                            }
                            echo "<br><img alt='' src='image/firstTaskImage.php'>";
                        }
                        ?>
                    </div>
                </div>
