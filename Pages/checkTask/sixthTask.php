...
$rule = array(
    "yRK" => "Метод Рунге-Кутта (значення)",
    "nArray" => "Метод Рунге-Кутта (крок)",
    "Miln" => "Метод Мілна"
);
        ...
        <div id="mid">
            ...
            <div id="posts">
                <div class="post">
                    <div class="story">
                        <?php
                        if (!$connect->isDone("6", $connect->getUserId($login, "users"), "taskdone")) {
                            $done = true;
                            $rezult = commaToPoint($_POST);
                            foreach ($rezult as $value) {
                                foreach ($value as $valuelow) {
                                    if ($valuelow == "" or ! is_numeric($valuelow)) {
                                        $done = false;
                                    }
                                }
                            }
                            if ($done) {
                                $data = $connect->getTaskRezult("6", $connect->getUserId($login, "users"), "tasktable");
                                $data = unserialize($data);
                                $done = TaskCheck::sixthTaskCheck($data, $rezult);
                                $rezult = serialize($rezult);
                                if ($connect->taskCheckExist("6", $connect->getUserId($login, "users"), "taskdone")) {
                                    $connect->taskCheckUpdate("6", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                } else {
                                    $connect->taskCheck("6", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                }
                            } else {
                            ...
                            $done = $connect->isDone("6", $connect->getUserId($login, "users"), "taskdone");
                            echo 'Завдання виконано<br>';
                        }
                        if ($done) {
                            ...
                            $dataSolver = $connect->getTaskRezult(6, $connect->getUserId($login, "users"), "tasktable");
                            ...
                            foreach ($dataSolver as $key => $value) {
                                ...
                                $key = rule($rule, $key);
                                echo "<br><div>$key<div><br><table class='task'><tr class='task'><td class='task'>x</td>";
                                ...
                                for ($index = 0; $index < count($yDataStudent); $index++) {
                                    echo '<td class="task">' . round($yDataStudent[$index], 7) . "</td>";
                                }
                            ...
                            echo "<br><img alt='' src='image/sixthTaskImage.php'>";
                        }
                        ?>
                    </div>
                </div>
