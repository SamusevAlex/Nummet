        ...
        <div id="mid">
            ...
            <div id="posts">
                <div class="post">
                    <div class="story">
                        <?php
                        if (!$connect->isDone("5", $connect->getUserId($login, "users"), "taskdone")) {
                            $done = true;
                            ...
                            if ($done) {
                                $data = $connect->getTaskRezult("5", $connect->getUserId($login, "users"), "tasktable");
                                $data = unserialize($data);
                                $done = TaskCheck::fifthTaskCheck($data, $rezult);
                                $rezult = serialize($rezult);
                                $rezult = preg_replace("/_/u", ".", $rezult);
                                if ($connect->taskCheckExist("5", $connect->getUserId($login, "users"), "taskdone")) {
                                    $connect->taskCheckUpdate("5", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                } else {
                                    $connect->taskCheck("5", $connect->getUserId($login, "users"), $rezult, $done, "taskdone");
                                }
                            } else {
                            ...
                            $done = $connect->isDone("5", $connect->getUserId($login, "users"), "taskdone");
                            echo 'Завдання виконано<br>';
                        }
                        if ($done) {
                            $dataStudent = $connect->getTaskRezult(5, $connect->getUserId($login, "users"), "taskdone");
                            $dataStudent = unserialize($dataStudent);
                            $dataSolver = $connect->getTaskRezult(5, $connect->getUserId($login, "users"), "tasktable");
                            ...
                            foreach ($dataSolver as $key => $value) {
                                ...
                                echo "<br><div>Для x = $key<div><br><table class='task'><tr class='task'><td class='task'>Похідна</td>";
                                ...
                                for ($index = 0; $index < count($yDataStudent); $index++) {
                                    echo '<td class="task">' . round($yDataStudent[$index], 5) . "</td>";
                                }
