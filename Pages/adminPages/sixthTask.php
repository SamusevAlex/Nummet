    ...
    <body>
        <div id="mid">
            ...
            <div id="posts">
                <div class="post">
                    <div class="story">
                        <h3>Форма вставки функції в завдання 6</h3>
                        <div>Приклад опису функції: (1+unk1*unk2)/pow(unk1,2)</div>
                        <div>Приклад опису функції для виведення: y'=(1+xy)/x^2</div>
                        <div>Приклад опису граничних умов: y0=0;x0=1</div><br>
                        <form action="" method="POST">
                            <div class='inTask'><label class='inputTaskFouth' for='n'>Функція</label><input type="text" id='n' class='function' name="func"></div>
                            <div class='inTask'><label class='inputTaskFouth' for='n2'>Функція для виведення</label><input type="text" id='n2' class='function' name="funcEcho"></div>
                            <div class='inTask'><label class='inputTaskFouth' for='n3'>Граничні умови</label><input type="text" id='n3' class='function' name="y0x0"></div>
                            <input type="submit" value="Вставити функцію" style="margin-left: 10%;">
                        </form>
                        <?php
                        if (isset($_POST['func'])) {
                            $func = htmlspecialchars($_POST['func']);
                            $funcEcho = htmlspecialchars($_POST['funcEcho']);
                            $y0x0 = htmlspecialchars($_POST['y0x0']);
                            $connect = new SqlConnectNumMet(HOST, USER, PASSWORD, DB);
                            $connect->addFunctionsSixthTask($func, $funcEcho, $y0x0, "functionsixthtask");
                        }
                        ?>
                    </div>
                </div>
