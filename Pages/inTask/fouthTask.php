                ...
                <div class="post">
                    <div class="story">
                        ...
                        if ($connect->isDone("4", $connect->getUserId($login, "users"), "taskdone")) {
                            echo '<div>Завдання виконано. <a href="../checkTask/fouthTask.php">Перейти до сторінки відповіді</a></div>';
                        } else {
                            $data = $connect->getTaskRezult("4", $connect->getUserId($login, "users"), "tasktable");
                            $data = unserialize($data);
                            var_dump($data);
                            echo '<form  action="../checkTask/fouthTask.php" method="POST" enctype="multipart/form-data">' . PHP_EOL;
                            foreach ($data as $key => $value) {
                                echo "<div class='inTask'>Результат для n = $key</div>" . PHP_EOL;
                                foreach ($value as $keylow => $valuelow) {
                                    $label = rule($rule, $keylow);
                                    echo "<div class='inTask'><label class='inputTask' for='$keylow$key'>$label </label><input type='text' id='$keylow$key' class='poleInputTask' name='$key" . "[$keylow]" . "[0]" . "'></div>" . PHP_EOL;
                                    echo "<div class='inTask'><label class='inputTask' for='d$keylow$key'>d </label><input type='text' id='d$keylow$key' class='poleInputTask' name='$key" . "[$keylow]" . "[1]" . "'></div>" . PHP_EOL;
                                }
                            }
                            echo '<input type="submit" value="Перевірити" style="margin-left: 4%;"></form>' . PHP_EOL;
                        }
                        ?>
                    </div>
                </div>
