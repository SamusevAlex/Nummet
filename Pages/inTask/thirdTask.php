                ...
                <div class="post">
                    <div class="story">
                        ...
                        if ($connect->isDone("3", $connect->getUserId($login, "users"), "taskdone")) {
                            echo '<div>Завдання виконано. <a href="../checkTask/thirdTask.php">Перейти до сторінки відповіді</a></div>';
                        } else {
                            $data = $connect->getTaskRezult("3", $connect->getUserId($login, "users"), "tasktable");
                            $data = unserialize($data);
                            //var_dump($data);
                            echo "<div>Введіть значення розраховані за допомогою кубічного сплайну в точках</div>";
                            echo '<form  action="../checkTask/thirdTask.php" method="POST" enctype="multipart/form-data">' . PHP_EOL;
                            foreach ($data as $key => $value) {
                                echo "<div class='inTask'><label class='inputTask' for='$key'>$key</label><input type='text' id='$key' class='poleInputTask' name='$key'></div>\n";
                            }
