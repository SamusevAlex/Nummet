                ...
                <div class="post">
                    <div class="story">
                        ...
                        if ($connect->isDone("5", $connect->getUserId($login, "users"), "taskdone")) {
                            echo '<div>Завдання виконано. <a href="../checkTask/fifthTask.php">Перейти до сторінки відповіді</a></div>';
                        } else {
                            $data = $connect->getTaskRezult("5", $connect->getUserId($login, "users"), "tasktable");
                            $data = unserialize($data);
                            var_dump($data);
                            echo '<form  action="../checkTask/fifthTask.php" method="POST" enctype="multipart/form-data">';
                            foreach ($data as $key => $value) {
                                echo "<div class='inTask'>Значення похідної в точці $key</div>";
                                foreach ($value as $keylow => $valuelow) {
                                    echo "<div class='inTask'><label class='inputTask' for='$key$keylow'>$keylow-а похідна </label><input type='text' id='$key$keylow' class='poleInputTask' name='$key" . "[$keylow]'></div>";
                                }
                            }
                            echo '<input type="submit" value="Перевірити" style="margin-left: 4%;"></form>';
                        }
                        ?>
                    </div>
                </div>
