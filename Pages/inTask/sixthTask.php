                ...
                <div class="post">
                    <div class="story">
                        ...
                        if ($connect->isDone("6", $connect->getUserId($login, "users"), "taskdone")) {
                            echo '<div>Завдання виконано. <a href="../checkTask/sixthTask.php">Перейти до сторінки відповіді</a></div>';
                        } else {
                            ...
                            $data = unserialize($data);
                            var_dump($data);
                            echo '<form  action="../checkTask/sixthTask.php" method="POST" enctype="multipart/form-data">' . PHP_EOL;
                            foreach ($data as $key => $value) {
                                $echo = rule($rule, $key);
                                echo "<div class='inTask'>$echo</div>" . PHP_EOL;
                                foreach ($value as $keylow => $valuelow) {
                                    echo "<div class='inTask six'><label class='inputTask' for='$key$keylow'>{$xData[$keylow]} </label><input type='text' id='$key$keylow' class='poleInputTask' name='$key" . "[$keylow]'></div>" . PHP_EOL;
                                }
                            }
