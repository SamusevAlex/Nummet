        ...
        <div id="mid">
            ...
            <div id="posts">
                <div class="post">
                    <div class="story">
                        <?php
                        if ($connect->isDone("1", $connect->getUserId($login, "users"), "taskdone")) {
                            echo '<div>Завдання виконано. <a href="../checkTask/firstTask.php">Перейти до сторінки відповіді</a></div>';
                        } else {
                            $data = $connect->getTaskRezult("1", $connect->getUserId($login, "users"), "tasktable");
                            $data = unserialize($data);
                            //var_dump($data);
                            echo '<form  action="../checkTask/firstTask.php" method="POST" enctype="multipart/form-data">' . PHP_EOL;
                            foreach ($data as $key => $value) {
                                echo "<div class='inTask'>Результат для полінома $key-го ступеня</div>" . PHP_EOL;
                                foreach ($value as $keylow => $valuelow) {
                                    echo "<div class='inTask'><label class='inputTask' for='$key$keylow'>$keylow </label><input type='text' id='$key$keylow' class='poleInputTask' name='$key" . "[$keylow]'></div>" . PHP_EOL;
                                }
                            }
